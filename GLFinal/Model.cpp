...
static uint32_t load_texture(std::filesystem::path filename) {
	...
	if (!image_data) { throw std::runtime_error("Failed to load image at path: " + filename.string()); }
    GLuint texture;
...
static void load_material_textures(aiMaterial* mat,
                                   aiTextureType type,
                                   std::vector<Texture>& textures) {
    for (std::size_t i = 0, texture_count = mat->GetTextureCount(type);
         i < texture_count; ++i) {
        aiString str;
        ...
        texture.type = (type == aiTextureType_DIFFUSE ? Texture_type::diffuse
...
static Mesh process_mesh(aiMesh* mesh, aiScene const* scene) {
    ...
    for (std::size_t i = 0; i < mesh->mNumVertices; ++i) {
        ...
        if (mesh->mTextureCoords[0]) {
            vertices[i].uv_coordinates = glm::vec2(mesh->mTextureCoords[0][i].x,
...
static void
process_node(std::vector<Mesh>& meshes, aiNode* node, aiScene const* scene) {
